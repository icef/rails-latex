class LatexToPdf
  ...
  #
  # lualatex is used to convert the file and creates the directory +#{Rails.root}/tmp/rails-latex/+ to store intermediate
  # files.
  #
...
class PdfGenerator
  DEFAULT_CONFIG = { :command => 'lualatex', :arguments => ['-halt-on-error'], :parse_twice => false }
  ...
  def handle_result
    if File.exist?(pdf_file)
      ...
      if File.exist?(log_file)
        exception = LatexBuildException.new("lualatex failed: See #{log_file} for details")
        exception.log_file = log_file
      else
        exception = UnknownLatexBuildException.new("lualatex failed for unknown reasons")
      end
      raise exception
    end
  end
